...
class ImportantNextGreaterElements {
    public static void main(String[] args) {
        // Sample test case 1
        int[] input1 = {4, 5, 2, 10, 8};
        int[] result1 = getNextGreaterElements(input1); // 5 10 10 -1 -1
        System.out.println("Next greater elements for {4, 5, 2, 10, 8}: ");
        printArray(result1);

        // Sample test case 2
        int[] input2 = {3, 7, 1, 7, 6, 4};
        int[] result2 = getNextGreaterElements(input2); // 7 -1 7 -1 -1 -1
        System.out.println("Next greater elements for {3, 7, 1, 7, 6, 4}: ");
        printArray(result2);

        // Sample test case 3
        int[] input3 = {6, 8, 0, 1, 3};
        int[] result3 = getNextGreaterElements(input3);// 8 -1 1 3 -1
    ...
    private static int[] getNextGreaterElements(int[] input) {
        ...
        stack.push(0);
        for (int i = 1; i < n; i++) {
            // current val greater than stack.peek()
        ...
        // if array is decreasing then stack also decreasing
        //edge case
        while (!stack.isEmpty()) {
            output[stack.pop()] = -1;
        }
